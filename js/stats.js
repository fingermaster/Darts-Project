if(inScope('stats')){
	var storage, vx, player, players, game, tempID, DB;
			...
			fxShotGame: function(game){			
				...
				request.openCursor(singleKeyRange).onsuccess = function(e) {
					...
					if (cursor) {
						...
						document.getElementById(`game_${game}_${(p1name == value.player)?'p1':'p2'}`).innerHTML += `<div class="shot ${(!correct)?'red':''}">${(value.sector!=0)? value.sector+outputMultiplier:'0'}</div>`;
